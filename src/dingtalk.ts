import { DingTalkConfig, DingTalkToken, DingTalkUserInfo, LoginResult } from './types.js';
...
export class DingTalkAuth {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        qrcode.generate(loginUrl, { small: true }, (qrCode) => {
          ...
          // 暂时返回模拟数据，待后续完善
          setTimeout(() => {
            const mockUser: DingTalkUserInfo = {
              openid: 'mock_openid',
   ...
   * 获取存储的 token
   */
  private getStoredToken(): DingTalkToken | null {
   ...
   * 保存 token
   */
  private saveToken(token: DingTalkToken): void {
