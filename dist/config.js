...
export function validateConfig(config) {
    ...
    const missingEndpoints = requiredEndpoints.filter(endpoint => !config.api?.endpoints?.[endpoint]);
    if (missingEndpoints.length > 0) {
        throw new Error(`关键接口端点缺失: ${missingEndpoints.join(', ')}`);
    }
    // 检查可选接口端点（给出警告但不阻止启动）
    const optionalEndpoints = ['getOSSToken'];
    const missingOptionalEndpoints = optionalEndpoints.filter(endpoint => !config.api?.endpoints?.[endpoint]);
 ...
 */
export function updateDingtalkConfig(appId, appSecret) {
