...
class IterationMCPServer {
    ...
    async handleBasicInfo(data) {
        try {
            ...
            try {
                ...
                const execOptions = { encoding: 'utf-8', cwd: workspaceRoot };
                // 检查工作目录是否是git仓库
                try {
                ...
                try {
                    ...
                    const projectName = pwd.split('/').pop()?.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
    ...
    async handleSubmitCompleteIteration(args) {
        try {
